...
class Urllib3Client(BaseClient):
        ...
        if request.proxy_host:
            ...
            if request.proxy_username:
                proxy_headers = urllib3.make_headers(
                    proxy_basic_auth=(
                        f"{request.proxy_username}" f":{request.proxy_password}"
                    )
                )
            else:
                proxy_headers = None
    ...
    def _process_request(self, request: Request):
        ...
        headers.setdefault(
            "User-Agent", bytes_to_str(request.user_agent or DEFAULT_USER_AGENT)
        )
        headers.setdefault(
            "Accept-Encoding", "gzip,deflate" if request.use_gzip else "none"
        )
        ...
        if request.auth_username is not None:
            headers.update(
                urllib3.util.make_headers(
                    basic_auth=(
                        f"{request.auth_username}" f":{request.auth_password or ''}"
                    )
                )
            )
